...
/* ─── Ava Avatar ─── */
function AvaAvatar({ avaState, onClickAva, isMobile }: { avaState: AvaState; onClickAva: () => void; isMobile: boolean }) {
  ...
  return (
    <div style={{ position:"relative", width:"100%", height:"100%", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"flex-end" }}>
      <style>{`
        ...
        @keyframes avaWave  { 0%,100%{transform:translateY(0) rotate(0)} 25%{transform:translateY(-18px) rotate(-4deg)} 75%{transform:translateY(-18px) rotate(4deg)} }
        @keyframes avaThink { 0%,100%{transform:translateY(0) rotate(0)} 50%{transform:translateY(4px) rotate(3deg)} }
        @keyframes avaHappy { 0%,100%{transform:translateY(0) scale(1)} 30%{transform:translateY(-22px) scale(1.04)} 60%{transform:translateY(-26px) scale(1.06)} }
      ...
      {bubble && (
        <div style={{ position:"absolute", top:16, right:isMobile?"auto":"108%", left:isMobile?"10px":"auto", background:"white", borderRadius:14, padding:"6px 12px", fontSize:12, fontWeight:600, color:"#1a1a1a", whiteSpace:"nowrap", boxShadow:"0 4px 16px rgba(0,0,0,0.18)", animation:"bubblePop 2.8s ease forwards", zIndex:10 }}>
          {bubble}
        </div>
      )}

      {avaState === "talking" && !isMobile && [0,0.42,0.84].map(d => (
      ...
      {reactions.map(r => (
        <div key={r.id} style={{ position:"absolute", top:"20%", left:"50%", fontSize:28, pointerEvents:"none", animation:"reactionPop 1.1s ease-out forwards" }}>{r.emoji}</div>
      ))}
      ...
      <div
        ...
        onClick={handleClick}
        style={{ cursor:"pointer", transition:"transform 0.1s ease-out", width:"100%", display:"flex", alignItems:"flex-end", justifyContent:"center", flex:1, overflow:"hidden" }}
      >
        <img
          src="/ava-avatar.png"
          alt="Ava"
...
export default function ChatPage() {
  ...
  async function sendMessage(text: string) {
    ...
    try {
      ...
      setMessages(prev => { const c=[...prev]; c[c.length-1]={role:"assistant",content:"API error — check that GROQ_API_KEY is set in Vercel → Settings → Environment Variables."}; return c; });
    } finally {
      setIsLoading(false);
      setAvaState("happy");
  ...
  const avatarWidth = isMobile ? "100%" : isTablet ? "clamp(180px,32%,280px)" : "clamp(240px,34%,420px)";
  const avatarHeight = isMobile ? "38vh" : "100%";

  return (
    <div style={{ height:"100dvh", display:"flex", flexDirection: isMobile ? "column" : "row", fontFamily:"'Segoe UI',system-ui,sans-serif", position:"relative", overflow:"hidden" }}>
      <BeachBackground />

      {/* AVA — top on mobile, right on desktop */}
      <div style={{
        width: isMobile ? "100%" : avatarWidth,
        height: avatarHeight,
        flexShrink: 0,
        order: isMobile ? 1 : 2,
        ...
        <AvaAvatar avaState={avaState} onClickAva={handleClickAva} isMobile={isMobile} />
      </div>

      {/* CHAT PANEL — below Ava on mobile, left on desktop */}
      <div style={{
        flex: 1,
        display:"flex",
        ...
        minHeight: 0,
        order: isMobile ? 2 : 1,
        padding: isMobile ? "8px 10px 0" : isTablet ? "14px 0 0 14px" : "20px 0 0 20px",
      }}>

        {/* Header */}
        <div style={{ background:"rgba(255,255,255,0.22)", backdropFilter:"blur(14px)", borderRadius:16, padding: isMobile ? "10px 14px" : "16px 22px", marginBottom:8, marginRight: isMobile ? 0 : 16, border:"1px solid rgba(255,255,255,0.4)", boxShadow:"0 4px 24px rgba(0,0,0,0.08)", flexShrink:0 }}>
          <h1 style={{ margin:0, fontSize: isMobile ? 20 : 26, fontWeight:900, letterSpacing:2, color:"#1a4a2e", textShadow:"0 1px 4px rgba(255,255,255,0.6)", whiteSpace:"nowrap" }}>Ava</h1>
          <p style={{ margin:"2px 0 0", fontSize: isMobile ? 11 : 13, color:"#2d5a3d", opacity:0.9, whiteSpace:"nowrap" }}>Your Bahamian Culture Guide 🌺</p>
        </div>

        {/* Messages */}
        <div style={{ flex:1, overflowY:"auto", background:"rgba(255,255,255,0.18)", backdropFilter:"blur(14px)", borderRadius:16, padding: isMobile ? "10px 12px" : "14px 18px", marginRight: isMobile ? 0 : 16, marginBottom:8, border:"1px solid rgba(255,255,255,0.38)", boxShadow:"0 4px 24px rgba(0,0,0,0.06)", display:"flex", flexDirection:"column", gap:8, minHeight:0 }}>
          {messages.length === 0 ? (
            <div style={{ paddingTop:4 }}>
              <p style={{ fontSize: isMobile ? 13 : 14, marginBottom:10, color:"#1a3a26", fontWeight:500 }}>Ask Ava anything about the Bahamas!</p>
              <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
                {SUGGESTED.map(q => (
                  <button key={q} onClick={() => sendMessage(q)}
                    style={{ background:"rgba(255,255,255,0.55)", border:"1px solid rgba(255,255,255,0.7)", color:"#1a4a2e", borderRadius:20, padding: isMobile ? "6px 12px" : "8px 16px", fontSize: isMobile ? 12 : 13, cursor:"pointer", fontWeight:500, backdropFilter:"blur(4px)", boxShadow:"0 2px 8px rgba(0,0,0,0.08)", textAlign:"left" }}
            ...
            <div key={i} style={{ display:"flex", justifyContent:msg.role==="user"?"flex-end":"flex-start", alignItems:"flex-end", gap:6 }}>
              {msg.role === "assistant" && (
                <div style={{ width:26, height:26, borderRadius:"50%", flexShrink:0, overflow:"hidden", border:"2px solid rgba(255,255,255,0.8)", boxShadow:"0 2px 8px rgba(0,0,0,0.15)" }}>
                  <img src="/ava-avatar.png" alt="Ava" style={{ width:"100%", height:"100%", objectFit:"cover", objectPosition:"top" }} />
                </div>
              )}
              <div style={{ maxWidth:"80%", padding: isMobile ? "9px 13px" : "11px 16px", borderRadius:msg.role==="user"?"18px 18px 4px 18px":"18px 18px 18px 4px", background:msg.role==="user"?"linear-gradient(135deg,#00897B,#005F99)":"rgba(255,255,255,0.88)", color:msg.role==="user"?"white":"#1a2a1e", fontSize: isMobile ? 13 : 14, lineHeight:1.65, boxShadow:"0 3px 12px rgba(0,0,0,0.12)", backdropFilter:msg.role==="assistant"?"blur(8px)":"none", border:msg.role==="assistant"?"1px solid rgba(255,255,255,0.6)":"none" }}>
                {msg.role === "assistant"
        ...
        <form onSubmit={e => { e.preventDefault(); sendMessage(input); }}
          style={{ display:"flex", gap:8, padding: isMobile ? "8px 10px" : "12px 14px", background:"rgba(255,255,255,0.28)", backdropFilter:"blur(14px)", borderRadius:16, marginRight: isMobile ? 0 : 16, marginBottom: isMobile ? 10 : 16, border:"1px solid rgba(255,255,255,0.5)", boxShadow:"0 4px 20px rgba(0,0,0,0.08)", flexShrink:0 }}>
          <input ref={inputRef} value={input} onChange={e => setInput(e.target.value)} placeholder="Ask about the Bahamas…" disabled={isLoading}
            style={{ flex:1, padding: isMobile ? "9px 14px" : "11px 18px", borderRadius:24, border:"1px solid rgba(255,255,255,0.7)", background:"rgba(255,255,255,0.7)", fontSize: isMobile ? 13 : 14, outline:"none", color:"#1a3a26", backdropFilter:"blur(4px)" }} />
          <button type="submit" disabled={!input.trim() || isLoading}
            style={{ padding: isMobile ? "9px 16px" : "11px 24px", borderRadius:24, border:"none", background:input.trim()&&!isLoading?"linear-gradient(135deg,#FFD700,#FF8C00)":"rgba(200,200,200,0.5)", color:input.trim()&&!isLoading?"#1a1a1a":"rgba(80,80,80,0.5)", fontWeight:700, fontSize: isMobile ? 13 : 14, cursor:input.trim()&&!isLoading?"pointer":"default", whiteSpace:"nowrap" }}>
            Send ➤
          </button>
        </form>
      </div>
