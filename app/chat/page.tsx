  ...
  return (
    <div style={{ position:"relative", width:"100%", height:"100%", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"flex-end" }}>
      <style>{`
        ...
        @keyframes avaWave  { 0%,100%{transform:translateY(0) rotate(0deg)} 25%{transform:translateY(-22px) rotate(-4deg)} 75%{transform:translateY(-22px) rotate(4deg)} }
        @keyframes avaThink { 0%,100%{transform:translateY(0) rotate(0deg)} 50%{transform:translateY(5px) rotate(3deg)} }
        ...
        @keyframes glowPulse { 0%,100%{filter:drop-shadow(0 0 18px rgba(0,220,200,0.7)) drop-shadow(0 0 50px rgba(0,155,119,0.4))} 50%{filter:drop-shadow(0 0 32px rgba(0,230,210,1)) drop-shadow(0 0 80px rgba(0,155,119,0.65))} }
      ...
      {/* Speech bubble */}
      {bubble && (
        <div style={{ position:"absolute", top:52, right:"108%", background:"white", borderRadius:14, padding:"8px 14px", fontSize:13, fontWeight:600, color:"#1a1a1a", whiteSpace:"nowrap", boxShadow:"0 4px 16px rgba(0,0,0,0.2)", animation:"bubblePop 2.8s ease forwards", zIndex:10 }}>
          {bubble}
      ...
      {avaState === "talking" && [0,0.42,0.84].map(d => (
        <div key={d} style={{ position:"absolute", bottom:"15%", left:"50%", width:200, height:200, borderRadius:"50%", border:"2px solid rgba(0,210,190,0.5)", pointerEvents:"none", animation:`glowRing 1.3s ease-out ${d}s infinite` }} />
      ))}
      ...
      >
        <img
          src="/ava-avatar.png"
          alt="Ava"
          style={{ maxHeight:"80vh", width:"auto", objectFit:"contain", objectPosition:"bottom center", display:"block", pointerEvents:"none", filter:"drop-shadow(0 8px 32px rgba(0,0,0,0.4))" }}
        />
      </div>

      <div style={{ fontSize:11, color:"rgba(255,255,255,0.4)", letterSpacing:0.5, marginBottom:8, marginTop:4 }}>
...
export default function ChatPage() {
  ...
  return (
    <div style={{ height:"100vh", background:"linear-gradient(180deg,#87CEEB 0%,#29ABE2 25%,#0086BF 55%,#005F99 80%,#003D66 100%)", display:"flex", fontFamily:"'Segoe UI',system-ui,sans-serif", position:"relative", overflow:"hidden" }}>
      ...
      {/* RIGHT — Ava photo */}
      <div style={{ width:"clamp(260px,35%,440px)", display:"flex", alignItems:"stretch", justifyContent:"center", position:"relative", zIndex:2, flexShrink:0 }}>
